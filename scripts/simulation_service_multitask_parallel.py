...
def evaluate_single_env(args_tuple):
    """
    Worker function to evaluate a single environment in a separate process.
    
    Args:
        args_tuple: Tuple containing (env_name, host, port, output_dir, n_episodes, 
                    n_envs, n_action_steps, max_episode_steps, task_index, total_tasks)
    
    Returns:
        dict: Result dictionary with evaluation metrics
    """
    (
        env_name,
        host,
    ...
    try:
        # Create simulation client in this process
        ...
        task_video_dir = os.path.join(output_dir, task_name)
        ...
        print(f"[Process {os.getpid()}] [{task_index+1}/{total_tasks}] Completed {env_name}: "
              f"Success rate: {success_rate:.3f} ({n_successes}/{n_episodes}), "
              f"Time: {evaluation_time:.1f}s")
        
        return result
        
    except Exception as e:
        print(f"[Process {os.getpid()}] [{task_index+1}/{total_tasks}] ERROR: Failed to evaluate {env_name}: {str(e)}")
        result = {
            'env_name': env_name,
            'success_rate': 0.0,
            'n_episodes': n_episodes,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--n_parallel",
        type=int,
        help="Number of parallel processes to use for multitask evaluation.",
        default=None,  # Default to CPU count if not specified
    )
    ...
    if args.server:
        ...
        if args.all_tasks:
            # Run evaluation on all 24 GR1 tasks in parallel
            print(f"Starting parallel evaluation on all {len(GR1_TASK_LIST)} GR1 tasks...")
            
            # Determine number of parallel processes
            if args.n_parallel is None:
                n_parallel = min(multiprocessing.cpu_count(), len(GR1_TASK_LIST))
            else:
                n_parallel = min(args.n_parallel, len(GR1_TASK_LIST))
            
            print(f"Using {n_parallel} parallel processes")
            ...
            task_args = []
            for i, env_name in enumerate(GR1_TASK_LIST):
                task_args.append((
                    env_name,
                    args.host,
                    args.port,
            ...
            # Run evaluations in parallel using Process (not Pool) to avoid daemon process issues
            # This allows AsyncVectorEnv to create child processes within each worker
            ...
            while task_index < len(task_args) or active_processes:
                # Start new processes up to n_parallel limit
                while len(active_processes) < n_parallel and task_index < len(task_args):
                    task_arg = task_args[task_index]
                    p = multiprocessing.Process(
                        target=lambda arg: result_queue.put(evaluate_single_env(arg)),
                        args=(task_arg,),
                        daemon=False  # Set to False to allow child processes (for AsyncVectorEnv)
                    )
                    p.start()
                    active_processes[p] = task_index
                    task_index += 1
            ...
            # Get any remaining results from queue
            try:
                while True:
                    result = result_queue.get_nowait()
                    results.append(result)
            ...
            # Sort results by task_index to maintain original order
            results = sorted(results, key=lambda x: x['task_index'])
            # Remove task_index from final results
